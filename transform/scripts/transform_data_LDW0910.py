...
def transform_time_row(row, task, progress):
    # Split the input row by the semicolon to separate timestamp and value
    timestamp_str, value_str = row.strip().split(';')

    # Parse the timestamp string into a pandas timestamp
    ...
    # Create a pandas series with the extracted components
    series = pd.Series({
        'datetime': timestamp,
        'YY': year,
        'MM': month,
        ...
        'HH': hour,
        'MN': minute,
        'Stat1': float(value_str.replace(',', '.'))  # Convert comma to period for decimal value
    })
