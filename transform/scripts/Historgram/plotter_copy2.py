...
class Plotter:
    def __init__(self, files,test, months, years,is_plot_test,  is_plot_month, is_plot_years):           # An dieser Stelle years,  is_plot_month, einfügen
        self.files = files
        self.months = months            
        self.years = years                      # NEW
        self.test = test
        self.is_plot_month = is_plot_month      # NEW
        self.is_plot_test = is_plot_test      #NEW TEST
    ...
    ####################################################
    ############## 1824####  WIRD NICHT GECALLED 
    def plot_test(self, year, month):
        if year is None or month is None:
            return
        self.task_plot_test = self.progress.add_task(f"[green]Plotting months graph...", total=len(self.months))

        file_name = f'{self.current_file_name}_year_{year}_month_{calendar.month_name[month]}'
        month_df = self.current_df[(self.current_df.index.year == year) & (self.current_df.index.month == month)]
        piv = pd.pivot_table(month_df, index=['DD'], columns=['YY'], values=['Stat1'], sort=False)
        piv.plot(figsize=(15, 7))
        plt.title(f'{self.current_file_name} {year} Month {calendar.month_name[month]}')
        plt.xlabel("Day of the Month")
        plt.ylabel("Value")
        plt.savefig(os.path.join(self.current_plot_folder, file_name))
...
############## Try 1741 ######################
    def plot_month(self):
        ...
        file_name = self.current_file_name + '_month' + 'test'
...
def main():
    ...
    Plotter(files=args.files, months=args.months, years=args.years, is_plot_years=args.years, is_plot_month=args.months, plot_month=args.months ).run()  # Original # is_plot_month=args.months & is_plot_test=args.test added
