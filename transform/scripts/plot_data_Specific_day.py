...
# Define the specific minimum and maximum values for each dataset
y_limits = [
    (0, 18),  # Dataset 1
    (0, 10),  # Dataset 2
    (0, 1300),  # Dataset 3
...
central_date = pd.Timestamp('2021-01-02')
start_date = central_date - pd.Timedelta(days=580)
end_date = central_date + pd.Timedelta(days=600) # bei Jahr 80
...
for i, column in enumerate(column_names_Discharge):
    ...
    # Set the y-axis limits using the specific values for the dataset
    y_min, y_max = y_limits[i]
    axs[i].set_ylim(y_min, y_max)  # Use specific min and max values
