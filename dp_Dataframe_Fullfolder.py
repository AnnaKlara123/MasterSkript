...
with open(filename, 'r') as csvfile:
    ...
    # csvreader = csv.reader(csvfile, delimiter = " ")  # Skipraws anpassen, je nachdem ab wann Datei eingelesen werden soll!
    data = pd.read_csv(filename, skiprows = 10, encoding='latin1', sep='\t', header=0)  # Funktioniert nicht, weil nicht möglich einzulese..  UnicodeDecodeError: 'utf-8' codec can't decode byte 0xfc in position 198: invalid start byte
    ## extracting field names/Headers through first row 
    ##fields = next(csvreader)
 
#     # extracting each data row one by one
#     for row in data:
print(data.loc[:300, 1])
#print(rows) 
...
# df.to_csv("dfCSVfilefull.csv", sep=' ', index=False)
